    ...
    run: async (client, message, args) => {
        ...
        await client.util.sleep(4000)
            interactionMessage.edit({
                embeds: [
                    embed.setDescription(
                        `**Are you sure you want to unhide all channels to in this server?**`
                    ),
                ], components: [row],
            });
                ...
                if (interaction.isButton()) {
                    if (interaction.customId === 'yes_button') {
                        interactionMessage.edit({
                            embeds: [
                                embed.setDescription(
                                    `${client.emoji.tick} | Successfully started unhiding to all channnels.`
                                ),
                            ],
                            components: [],
                        });
                        ...
                        try {
                           ...
                           if (message.guild.Unhideall) message.guild.Unhideall = false;
    ...
    if (collected.size === 0) {
        interactionMessage.edit({
            ...
            return message.channel.send({
                embeds: [
                    embed.setDescription(
                        `${client.emoji.cross} | There is already a Unhide all process is going on in the server.`)
                        .setColor(client.color),
                ],
            });
        }
